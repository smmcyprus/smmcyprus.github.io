<?php
function coinmotion_get_widget_data($widget = COINMOTION_OPTION_NAME_WIDGET_0) {
	$options = coinmotion_get_option($widget);
	$db_data = coinmotion_get_default_data($widget);
	
	/*$lastKey = coinmotion_get_lastkey($widget);
	if (! empty($lastKey)){*/
...
function coinmotion_get_widget_rate_period_data($widget = COINMOTION_OPTION_NAME_WIDGET_RATE_PERIOD) {
	$options = coinmotion_get_option($widget);
	$db_data = coinmotion_get_default_data($widget);
	/*$lastKey = coinmotion_get_lastkey($widget);
	if (! empty($lastKey)){*/
...
function coinmotion_get_widget_currency_details_data($widget = COINMOTION_OPTION_NAME_WIDGET_CURRENCY_DETAILS) {
	$options = coinmotion_get_option($widget);
	$db_data = coinmotion_get_default_data($widget);
	/*$lastKey = coinmotion_get_lastkey($widget);
	if (! empty($lastKey)){*/
...
function coinmotion_get_widget_currency_conversor_data($widget = COINMOTION_OPTION_NAME_WIDGET_CURRENCY_CONVERSOR) {
	$options = coinmotion_get_option($widget);
	$db_data = coinmotion_get_default_data($widget);
	/*$lastKey = coinmotion_get_lastkey($widget);
	if (! empty($lastKey)){*/
...
function coinmotion_get_default_data($widget = COINMOTION_OPTION_NAME_WIDGET_0){
	$params = [];
	switch ($widget){
