...
public function  ccpw_shortcode( $atts, $content = null ) {
    ...
    $credit_html	=	'<div class="ccpw-credits"><a href="https://www.coingecko.com" target="_blank" rel="nofollow">'.__('Powered by CoinGecko API','ccpw').'</a></div>';
    ...
    switch($type){
       ...
       if($getRecords >$datatable_pagination){
        ...
        $limit=$getRecords;
            ...
            data-currency-rate="'.ccpw_usd_conversions($fiat_currency).'" 
...
function ccpw_enqueue_assets($type, $post_id){
	...
	switch($type){
		case 'ticker':
			...
			wp_enqueue_script('ccpw_bxslider_js', '//cdn.jsdelivr.net/bxslider/4.2.12/jquery.bxslider.js', array('jquery'), CCPWF_VERSION, true);
